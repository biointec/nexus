...
/** A simple SelectZero implementation based on a two-level inventory, a spill
 * list and broadword bit search.
 *
...
template <util::AllocType AT = util::AllocType::MALLOC> class SimpleSelectZero {
  private:
    ...
    int log2_zeros_per_inventory, log2_zeros_per_sub16, log2_zeros_per_sub64,
        log2_longwords_per_subinventory, zeros_per_inventory, zeros_per_sub16,
        zeros_per_sub64, longwords_per_subinventory, longwords_per_inventory,
        zeros_per_inventory_mask, zeros_per_sub16_mask, zeros_per_sub64_mask;
  ...
  public:
    ...
    SimpleSelectZero(const uint64_t* const bits, const uint64_t num_bits,
        ...
        zeros_per_inventory =
            num_bits == 0
                ? 0
                : (c * max_zeros_per_inventory + num_bits - 1) / num_bits;
        ...
        log2_longwords_per_subinventory =
            min(max_log2_longwords_per_subinventory,
                max(0, log2_zeros_per_inventory - 2));
        longwords_per_subinventory = 1 << log2_longwords_per_subinventory;
        longwords_per_inventory = longwords_per_subinventory + 1;
        log2_zeros_per_sub64 =
            max(0, log2_zeros_per_inventory - log2_longwords_per_subinventory);
            ...
            for (int j = 0; j < 64; j++) {
                ...
                if (~bits[i] & 1ULL << j) {
                    if ((d & zeros_per_inventory_mask) == 0)
                        inventory[(d >> log2_zeros_per_inventory) *
        ...
        if (exact_spill_size > 0)
            printf("First spilled entries: %016" PRIx64 " %016" PRIx64
                   " %016" PRIx64 " %016" PRIx64 "\n",
                   exact_spill[0], exact_spill[1], exact_spill[2],
                   exact_spill[3]);
#endif
    }
        ...
        if (inventory_rank >= 0) {
            start = inventory_rank +
                    ((uint16_t*)(inventory_start +
                                 1))[subrank >> log2_zeros_per_sub16];
