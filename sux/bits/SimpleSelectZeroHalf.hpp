...
class SimpleSelectZeroHalf {
  private:
    ...
    static const int log2_zeros_per_sub64 =
        log2_zeros_per_inventory - log2_longwords_per_subinventory;
  ...
  public:
...
#ifdef DEBUG
        printf("Number of bits: %" PRId64 " Number of zeros: %" PRId64
               " (%.2f%%)\n",
    ...
    uint64_t selectZero(const uint64_t rank) {
        ...
        if (inventory_rank >= 0) {
            ...
            assert((subrank >> log2_zeros_per_sub64) <
                   longwords_per_subinventory);
            start = -inventory_rank - 1 +
                    *(inventory_start + 1 + (subrank >> log2_zeros_per_sub64));
