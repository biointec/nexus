...
class SimpleSelectHalf : public Select {
  private:
    ...
    static const int longwords_per_subinventory =
        1 << log2_longwords_per_subinventory;
    static const int log2_ones_per_sub64 =
        log2_ones_per_inventory - log2_longwords_per_subinventory;
  ...
  public:
    ...
    SimpleSelectHalf(std::ifstream& ifs, const uint64_t* const bits)
        : bits(bits) {
...
#ifdef DEBUG
        printf("Number of bits: %" PRId64 " Number of ones: %" PRId64
               " (%.2f%%)\n",
            ...
            for (int j = 0; j < 64; j++) {
                ...
                if (bits[i] & 1ULL << j) {
                    if ((d & mask) == 0) {
                        ...
                        span = inventory[inventory_index +
                                         longwords_per_subinventory + 1] -
                               start;
                        if (span > (1 << 16))
    ...
    uint64_t select(const uint64_t rank) {
        ...
        if (inventory_rank >= 0) {
            ...
            assert((subrank >> log2_ones_per_sub64) <
                   longwords_per_subinventory);
            start = -inventory_rank - 1 +
                    *(inventory_start + 1 + (subrank >> log2_ones_per_sub64));
    ...
    size_t bitCount() const {
        return inventory.bitCount() - sizeof(inventory) * 8 + sizeof(*this) * 8;
    };
};
