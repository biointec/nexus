 ...
 *
 * This inferface specifies primitives both on ones and on zeroes because
 * ranking on ones automatically makes it possible ranking on zeros, and
 * viceversa. This interface provides template methods for rank(std::size_t,
 * std::size_t), rankZero(std::size_t) and rankZero(std::size_t,std::size_t)
...
class Rank {
  public:
     ...
     * @return the number of ones from the starting position (included) to the
     ...
     *
     * @param pos A position from 0 to size() (included).
     * @return the number of zeros from the start of the bit vector up to the
     * given position (excluded).
     */
     ...
     * @param from A starting position from 0 to size() (included).
     * @param to An ending position from `from` to size() (included).
     * @return the number of zeros from the starting position (included) to the
     * ending position (excluded).
     */
    uint64_t rankZero(std::size_t from, std::size_t to) {
        return rankZero(to) - rankZero(from);
    }
