...
/** A simple Select implementation based on a two-level inventory, a spill list
 * and broadword bit search.
 *
 * This implementation uses around 13.75% additional space on evenly distributed
 * bit arrays, and, under the same conditions, provide very fast selects. For
 * very unevenly distributed arrays the space occupancy will grow significantly,
 * and access time might vary wildly.
 *
...
class SimpleSelect : public Select {
  private:
    ...
    int log2_ones_per_inventory, log2_ones_per_sub16, log2_ones_per_sub64,
        log2_longwords_per_subinventory, ones_per_inventory, ones_per_sub16,
        ones_per_sub64, longwords_per_subinventory, longwords_per_inventory,
  ...
  public:
    ...
    virtual void write(std::ofstream& ofs) const {
        // write the ints
        write_int(ofs, log2_ones_per_inventory);
        write_int(ofs, log2_ones_per_sub16);
        write_int(ofs, log2_ones_per_sub64);
        write_int(ofs, log2_longwords_per_subinventory);
        write_int(ofs, ones_per_inventory);
        write_int(ofs, ones_per_sub16);
        write_int(ofs, ones_per_sub64);
        write_int(ofs, longwords_per_subinventory);
        write_int(ofs, longwords_per_inventory);
        write_int(ofs, ones_per_inventory_mask);
        write_int(ofs, ones_per_sub16_mask);
        write_int(ofs, ones_per_sub64_mask);

        write_uint64_t(ofs, num_words);
        write_uint64_t(ofs, inventory_size);
        write_uint64_t(ofs, exact_spill_size);
        write_uint64_t(ofs, num_ones);

        // write the vectors;
        ofs << inventory;
        ofs << exact_spill;

        // bits are constant pointer and not the responsibility of this class
    }

    SimpleSelect(std::ifstream& ifs, const uint64_t* const bits) : bits(bits) {
        // write the ints
        read_int(ifs, log2_ones_per_inventory);
        read_int(ifs, log2_ones_per_sub16);
        read_int(ifs, log2_ones_per_sub64);
        read_int(ifs, log2_longwords_per_subinventory);
        read_int(ifs, ones_per_inventory);
        read_int(ifs, ones_per_sub16);
        read_int(ifs, ones_per_sub64);
        read_int(ifs, longwords_per_subinventory);
        read_int(ifs, longwords_per_inventory);
        read_int(ifs, ones_per_inventory_mask);
        read_int(ifs, ones_per_sub16_mask);
        read_int(ifs, ones_per_sub64_mask);

        read_uint64_t(ifs, num_words);
        read_uint64_t(ifs, inventory_size);
        read_uint64_t(ifs, exact_spill_size);
        read_uint64_t(ifs, num_ones);
     ...
     * @param max_log2_longwords_per_subinventory the number of words per
     * subinventory: a larger value yields a faster map that uses more space;
     * typical values are between 0 and 3.
     */
    SimpleSelect(const uint64_t* const bits, const uint64_t num_bits,
        ...
        ones_per_inventory =
            num_bits == 0
                ? 0
                : (c * max_ones_per_inventory + num_bits - 1) / num_bits;
        ...
        log2_longwords_per_subinventory =
            min(max_log2_longwords_per_subinventory,
                max(0, log2_ones_per_inventory - 2));
        longwords_per_subinventory = 1 << log2_longwords_per_subinventory;
        longwords_per_inventory = longwords_per_subinventory + 1;
        log2_ones_per_sub64 =
            max(0, log2_ones_per_inventory - log2_longwords_per_subinventory);
        ...
        const int64_t* end_of_inventory =
            &inventory + inventory_size * longwords_per_inventory + 1;
            ...
            for (int j = 0; j < 64; j++) {
                ...
                if (bits[i] & 1ULL << j) {
                    if ((d & ones_per_inventory_mask) == 0)
                        inventory[(d >> log2_ones_per_inventory) *
                                  longwords_per_inventory] = i * 64 + j;
                    d++;
                }
            }
                    ...
                    if (bits[i] & 1ULL << j) {
                        if ((d & ones_per_inventory_mask) == 0) {
                            ...
                            start = inventory[inventory_index *
                                              longwords_per_inventory];
                            span = inventory[(inventory_index + 1) *
                                             longwords_per_inventory] -
                                   start;
                            ones = min(c - d, (uint64_t)ones_per_inventory);

                            assert(start + span == num_bits ||
                                   ones == ones_per_inventory);

                            // We accumulate space for exact pointers ONLY if
                            // necessary.
                            if (span >= (1 << 16)) {
                                exact += ones;
                                if (ones_per_sub64 > 1)
...
#ifdef DEBUG
        // printf("First inventories: %" PRId64 " %" PRId64 " %" PRId64 " %"
        // PRId64 "\n", inventory[0], inventory[1], inventory[2],
        //       inventory[3]);
        // if ( subinventory_size > 0 ) printf("First subinventories: %016"
        // PRIx64 " %016" PRIx64 " %016" PRIx64 " %016" PRIx64 "\n",
        // subinventory[ 0 ], subinventory[ 1 ], subinventory[ 2 ],
        // subinventory[ 3 ] );
        // if (exact_spill_size > 0)
        //  printf("First spilled entries: %016" PRIx64 " %016" PRIx64 " %016"
        //  PRIx64 " %016" PRIx64 "\n", exact_spill[0],
    ...
    size_t select(const uint64_t rank) {
        ...
        const int64_t* inventory_start =
            &inventory + (inventory_index << log2_longwords_per_subinventory) +
            inventory_index;
...
#ifdef DEBUG
        printf("Rank: %" PRId64 " inventory index: %" PRId64
               " inventory rank: %" PRId64 " subrank: %d\n",
        ...
        if (inventory_rank >= 0) {
            start = inventory_rank +
                    ((uint16_t*)(inventory_start +
                                 1))[subrank >> log2_ones_per_sub16];
            residual = subrank & ones_per_sub16_mask;
        } else {
            if (ones_per_sub64 == 1)
                return *(inventory_start + 1 + subrank);
            assert(*(inventory_start + 1) + subrank <
                   (int64_t)exact_spill_size);
    ...
    size_t bitCount() const {
        return inventory.bitCount() - sizeof(inventory) * 8 +
               exact_spill.bitCount() - sizeof(exact_spill) * 8 +
               sizeof(*this) * 8;
    }
};
