...
class Rank9Sel : public Rank9<AT>, public Select {
  private:
  ...
  public:
    ...
    Rank9Sel(std::ifstream& ifs, const uint64_t* const bits,
             const uint64_t num_bits)
        : Rank9<AT>(ifs, bits, num_bits) {
        // write the ints
    ...
    size_t select(const uint64_t rank) {
...
#ifdef DEBUG
        printf("Initially, rank: %" PRId64 " block_left: %" PRId64
               " block_right: %" PRId64 " span: %" PRId64 "\n",
        ...
        if (span < 2) {
...
#ifdef DEBUG
            printf("Single span; rank_in_block: %" PRId64
                   " block_left: %" PRId64 "\n",
            ...
            assert(where <= 16);
#ifdef DEBUG
            printf("rank_in_superblock: %" PRId64 " (%" PRIx64 ") %" PRIx64
                   " %" PRIx64 "\n",
...
#ifdef DEBUG
            printf("Found where (1): %d rank_in_block: %" PRId64
                   " block_left: %" PRId64 "\n",
...
#ifdef DEBUG
            printf("Found where (2): %d rank_in_block: %" PRId64
                   " block_left: %" PRId64 "\n",
...
#ifdef DEBUG
        printf("rank_in_block: %" PRId64 " offset_in_block: %" PRId64
               " rank_in_word: %" PRId64 "\n",
    ...
    size_t bitCount() const {
        return this->counts.bitCount() - sizeof(this->counts) * 8 +
               inventory.bitCount() - sizeof(inventory) * 8 +
               subinventory.bitCount() - sizeof(subinventory) * 8 +
               sizeof(*this) * 8;
    }
};
