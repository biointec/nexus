...
class Rank9 : public Rank {
  ...
  public:
    virtual void write(std::ofstream& ofs) const {
        ...
        // write the ints
        ofs.write((char*)&num_ones, sizeof(num_ones));
        ofs << counts;

        // bits are constant pointer and not the responsibility of this class
    }

    Rank9(std::ifstream& ifs, const uint64_t* const bits,
          const uint64_t num_bits)
        : num_bits(num_bits), bits(bits) {
        ifs.read((char*)&num_ones, sizeof(num_ones));
        ifs >> counts;
    }
    ...
    size_t bitCount() const {
        return counts.bitCount() - sizeof(counts) * 8 + sizeof(*this) * 8;
    }
