...
/** An implementation of selection and ranking based on the Elias-Fano
 * representation of monotone sequences.
 *
 * Instances of this class can be built using a bit vector or an explicit list
 * of positions for the ones in a vector. In every case, the bit vector or the
 * list are not necessary after construction.
 *
...
class EliasFano : public Rank, public Select {
  private:
    ...
    __inline static void set(util::Vector<uint64_t, AT>& bits,
                             const uint64_t pos) {
        bits[pos / 64] |= 1ULL << pos % 64;
    }

    __inline static uint64_t get_bits(util::Vector<uint64_t, AT>& bits,
                                      const uint64_t start, const int width) {
        ...
        return (total_offset <= 64
                    ? result
                    : result | bits[start_word + 1] << (64 - start_bit)) &
               ((1ULL << width) - 1);
    }

    __inline static void set_bits(util::Vector<uint64_t, AT>& bits,
                                  const uint64_t start, const int width,
  ...
  public:
    void write(ofstream& ofs) const {
        ...
        write_int(ofs, l);
        write_int(ofs, block_size);
        write_int(ofs, block_length);

        ofs << lower_bits;
        ofs << upper_bits;

        select_upper.write(ofs);

        // bits are constant pointer and not the responsibility of this class
    }

    EliasFano(std::ifstream& ifs, const uint64_t num_bits)
        : num_bits(num_bits) {

        read_uint64_t(ifs, num_ones);

        read_int(ifs, l);
        read_int(ifs, block_size);
        read_int(ifs, block_length);

        ifs >> lower_bits;
        ifs >> upper_bits;

        select_upper = SimpleSelectHalf<>(ifs, &upper_bits);
    }
    ...
    EliasFano(const uint64_t* const bits, const uint64_t num_bits) {
        ...
        for (uint64_t i = 0; i < num_bits; i++) {
            if (bits[i / 64] & (1ULL << i % 64)) {
                if (l != 0)
                    set_bits(lower_bits, pos * l, l, i & lower_bits_mask);
        ...
        select_upper =
            SimpleSelectHalf<>(&upper_bits, num_ones + (num_bits >> l));
        selectz_upper =
            SimpleSelectZeroHalf<>(&upper_bits, num_ones + (num_bits >> l));
     ...
     *  representation of the given list. select(const uint64_t rank) will
     * retrieve an element of the list, and rank(const size_t pos) will return
     * how many element of the list are smaller than the argument.
     *
...
#ifdef DEBUG
        printf("First lower: %016llx %016llx %016llx %016llx\n", lower_bits[0],
               lower_bits[1], lower_bits[2], lower_bits[3]);
        printf("First upper: %016llx %016llx %016llx %016llx\n", upper_bits[0],
               upper_bits[1], upper_bits[2], upper_bits[3]);
#endif
    ...
    uint64_t rank(const size_t k) {
        ...
        while (rank > block_size) {
            ...
            const uint64_t cmp = ((((block_lower_bits | msbs_step_l) -
                                    (k_lower_bits_step_l & ~msbs_step_l)) |
                                   (k_lower_bits_step_l ^ block_lower_bits)) ^
                                  (k_lower_bits_step_l & ~block_lower_bits)) &
                                 msbs_step_l;

            // printf( "Compare: %016llx compressed: %016llx shifted:
            // %016llx\n", cmp, cmp * compressor, cmp
            // * compressor >> block_size * l );

            const uint64_t cmp_compr =
                ~(cmp * compressor >> block_length & block_upper_bits) &
                block_size_mask;
    ...
    size_t select(const uint64_t rank) {
        ...
        printf("Returning %lld = %llx << %d | %llx\n",
               (select_upper.select(rank) - rank) << l |
                   get_bits(lower_bits, rank * l, l),
               select_upper.select(rank) - rank, l,
               get_bits(lower_bits, rank * l, l));
#endif
        return (select_upper.select(rank) - rank) << l |
               get_bits(lower_bits, rank * l, l);
    }
    ...
    uint64_t bitCount() {
        return upper_bits.bitCount() - sizeof(upper_bits) * 8 +
               lower_bits.bitCount() - sizeof(lower_bits) * 8 +
               select_upper.bitCount() - sizeof(select_upper) * 8 +
               selectz_upper.bitCount() - sizeof(selectz_upper) * 8 +
               sizeof(*this) * 8;
    }
};
