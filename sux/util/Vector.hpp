...
enum AllocType {
    ...
    MALLOC,
    /** Allocation with `mmap()`. Allocations are aligned on a memory page
       (typically, 4KiB). */
    SMALLPAGE,
    /** Transparent huge pages support through `mmap()` and `madvise()`
     * on Linux. Allocations are usually a mutiple of 4KiB, but they can be
     * defragmented in blocks of 2MiB each. */
    TRANSHUGEPAGE,
    /** Direct huge page support through `mmap()` on Linux.
     * In this case allocations are aligned on a huge (typically, 2MiB) memory
     * page. This feature is usually disabled by default and it requires the
     * administrator
     * to pre-reserve space for huge memory pages as documented in the reported
     * external references  */
    FORCEHUGEPAGE
};
...
#define MADV_HUGEPAGE 0
  ...
  public:
    ...
    static constexpr int FLAGS =
        MAP_PRIVATE | MAP_ANONYMOUS | (AT == FORCEHUGEPAGE ? MAP_HUGETLB : 0);
  ...
  public:
    ...
    explicit Vector<T, AT>(size_t length) {
        size(length);
    }

    explicit Vector<T, AT>(const T* data, size_t length) : Vector(length) {
    ...
    Vector(Vector<T, AT>&& oth)
        : _size(std::exchange(oth._size, 0)),
          _capacity(std::exchange(oth._capacity, 0)),
    ...
    /** Trim the the memory allocated so that it holds at most the given number
     * of elements.
    ...
    /** Enlarges the backing array to that it can contain a given number of
     * elements.
     *
    ...
    /** Enlarges the backing array to that it can contain a given number of
     * elements, plus possibly extra space.
     *
     * If the current capacity is sufficient, nothing happens. Otherwise, the
    ...
    void grow(size_t capacity) {
        if (capacity > _capacity)
            remap(max(capacity, _capacity + (_capacity / 2)));
    }
    ...
    size_t bitCount() const {
        return sizeof(*this) * 8 + _capacity * sizeof(T) * 8;
    }
  ...
  private:
    ...
    void remap(size_t size) {
...
#else
                mem = mremap(data, page_aligned(_capacity), space,
                             MREMAP_MAYMOVE, -1, 0);
#endif
            }
        ...
        if (_capacity * sizeof(T) < space)
            memset(static_cast<char*>(mem) + _capacity * sizeof(T), 0,
                   space - _capacity * sizeof(T));
